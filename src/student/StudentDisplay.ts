...
export class StudentDisplay {
  ...
  public async displayStudents(): Promise<void> {
    ...
    if (shouldSort.trim().toLowerCase() === "y") {
      const field = await this.getSortField();

      const order = await this.getSortOrder();

      const sortedStudents = this.studentSorter.sortBy(students, field, order);

      this.displayStudentTable(sortedStudents);
